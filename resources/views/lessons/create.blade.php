...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-slate-800/50 border border-white/5 rounded-2xl backdrop-blur-sm p-6 sm:p-8">
        <form action="{{ route('courses.lessons.store', $course) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="space-y-6">
                <div>
                    <label for="title" class="block text-sm font-semibold text-slate-300 mb-2">Dars nomi <span
                            class="text-rose-500">*</span></label>
                    <input type="text" id="title" name="title" value="{{ old('title') }}"
                        class="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none @error('title') border-rose-500/50 @enderror"
                ...
                <div>
                    <label for="video_url" class="block text-sm font-semibold text-slate-300 mb-2">Video URL
                        (YouTube)</label>
                    <input type="url" id="video_url" name="video_url" value="{{ old('video_url') }}"
                        class="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none @error('video_url') border-rose-500/50 @enderror"
                ...
                <div>
                    ...
                    <input type="file" name="file"
                        class="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white file:mr-4 file:py-1 file:px-4 file:rounded-lg file:border-0 file:bg-blue-500/20 file:text-blue-400 file:hover:bg-blue-500/30 file:transition-colors" />
                    @error('file')
                    <p class="text-rose-400 text-xs mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label for="order" class="block text-sm font-semibold text-slate-300 mb-2">Tartib
                            raqami</label>
                        <input type="number" id="order" name="order" value="{{ old('order', $nextOrder) }}"
                            class="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none"
                            required />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            ...
                            <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>Nofaol
                            </option>
                        </select>
                    </div>
                </div>
