...
<body
    ...
    <div x-data="{ sidebarOpen: true }" class="flex h-screen w-full relative">
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                <div class="p-5 border-b border-white/5">
                    <a href="{{ route('courses.lessons.index', $course) }}"
                        class="text-sm text-slate-400 hover:text-white transition-colors flex items-center gap-1 mb-3">
        ...
        <button @click="sidebarOpen = !sidebarOpen"
            ...
            <svg x-show="sidebarOpen" class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-900/50">
            {{-- Top Bar --}}
            <header
                class="h-16 flex items-center justify-between px-6 bg-slate-900/80 backdrop-blur-md border-b border-white/5 flex-shrink-0">
                <div class="flex items-center gap-3">
                    <span
                        class="w-8 h-8 bg-blue-500/10 rounded-lg flex items-center justify-center text-blue-400 font-bold text-sm">
                        {{ $lesson->order }}
                    </span>
