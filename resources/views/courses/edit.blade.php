...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-white">Kursni tahrirlash: <span
                class="text-blue-400">{{ $course->title }}</span></h2>
    </div>
        ...
        <form action="{{ route('courses.update', $course) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <div x-data="{
                    ...
                    <template x-if="hasImage && preview">
                        <div class="relative w-full h-48 rounded-xl overflow-hidden border border-white/10">
                            ...
                            <button type="button" @click="removeImage()"
                                class="absolute top-3 right-3 bg-rose-500/80 hover:bg-rose-500 text-white p-2 rounded-lg transition-colors">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-xs text-slate-500">Yangi rasm yuklasangiz, joriy rasm avtomatik
                            almashtiriladi</p>
                    </div>
                    @endif
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    ...
                    placeholder="Kurs haqida qisqacha...">{{ old('description', $course->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select id="status" name="status"
                    class="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all @error('status') border-rose-500/50 @enderror"
                    required>
                    <option value="draft" {{ old('status', $course->status) == 'draft' ? 'selected' : '' }}>Qoralama
                    </option>
                    <option value="active" {{ old('status', $course->status) == 'active' ? 'selected' : '' }}>Faol
                    </option>
