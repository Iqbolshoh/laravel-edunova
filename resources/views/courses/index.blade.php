...
<div class="space-y-6" x-data="{ deleteModalOpen: false, deleteUrl: '', deleteTitle: '' }">
            ...
            <div x-show="deleteModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-2xl bg-slate-800/95 border border-white/10 text-left shadow-2xl shadow-rose-500/10 transition-all sm:my-8 sm:w-full sm:max-w-md p-6">
                    ...
                    <div class="mt-5 text-center sm:mt-6">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-slate-400">
                                <span x-text="deleteTitle"></span> kursini o'chirmoqchimisiz? Bu amal qaytarib bo'lmaydi va barcha darslar bilan birga o'chiriladi.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="mt-6 flex flex-col sm:flex-row-reverse gap-3 sm:gap-4">
                    <form :action="deleteUrl" method="POST" class="w-full sm:w-auto flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full justify-center rounded-xl bg-rose-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-rose-500/30 hover:bg-rose-400 transition-colors">Ha, o'chirish</button>
                    </form>
                    <button type="button" @click="deleteModalOpen = false" class="mt-3 sm:mt-0 w-full sm:w-auto flex-1 justify-center rounded-xl bg-slate-700/50 px-4 py-2.5 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-slate-600 hover:bg-slate-700 transition-colors">Yo'q, bekor qilish</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div x-data="{ show: true }" x-show="show" class="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 text-emerald-400">
        ...
        <button @click="show = false" class="text-emerald-400 hover:text-emerald-300"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-slate-800/50 border border-white/5 rounded-2xl overflow-hidden hover:border-blue-500/30 transition-all duration-300 hover:-translate-y-1 group">
            ...
            <div class="p-6">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('courses.show', $course) }}" class="flex-1 text-center bg-slate-700/50 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-lg text-sm transition-colors">Ko'rish</a>
                    @can('courses.edit')
                    <a href="{{ route('courses.edit', $course) }}" class="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 px-3 py-2 rounded-lg text-sm transition-colors cursor-pointer" title="Tahrirlash">
                    ...
                    @endcan
                    @can('courses.delete')
                    <button @click="deleteUrl = '{{ route('courses.destroy', $course) }}'; deleteTitle = '{{ $course->title }}'; deleteModalOpen = true" class="bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 px-3 py-2 rounded-lg text-sm transition-colors cursor-pointer" title="O'chirish">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
