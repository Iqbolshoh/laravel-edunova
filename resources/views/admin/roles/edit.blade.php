...
@section('header_title', 'Rollar / Tahrirlash')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-white">Rolni tahrirlash: <span class="text-blue-400">{{ ucfirst($role->name) }}</span></h2>
    </div>
    ...
    <div class="bg-slate-800/50 border border-white/5 rounded-2xl backdrop-blur-sm p-6 sm:p-8">
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
            ...
            <div class="mb-8">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" class="w-full bg-slate-900/50 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
                @error('name')
                        ...
                        <div class="space-y-3">
                            ...
                            <label class="flex items-start gap-3 cursor-pointer group">
                                <div class="relative flex items-center pt-0.5">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="peer h-4 w-4 cursor-pointer appearance-none rounded border border-slate-600 bg-slate-800 checked:border-blue-500 checked:bg-blue-500 transition-all" @if($isChecked) checked @endif>
