...
@section('header_title', 'Foydalanuvchilarni boshqarish')
...
<div x-data="{ deleteModalOpen: false, deleteUrl: '' }">
            ...
            <div x-show="deleteModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-3xl bg-slate-800/95 border border-white/10 text-left shadow-2xl shadow-rose-500/10 transition-all sm:my-8 sm:w-full sm:max-w-md p-8">
                <div>
                    ...
                    <div class="mt-5 text-center sm:mt-6">
                        <h3 class="text-xl font-semibold leading-6 text-white">Foydalanuvchini o'chirish</h3>
                        <div class="mt-2">
                            <p class="text-sm text-slate-400">Haqiqatan ham bu foydalanuvchini o'chirmoqchimisiz? Davom etsangiz, bu ma'lumotlarni qayta tiklab bo'lmaydi.</p>
                        </div>
                    </div>
                </div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-white">Foydalanuvchilar</h2>
            <p class="text-sm text-slate-400 mt-1">Tizimdagi barcha foydalanuvchilar va ularga biriktirilgan rollar</p>
        </div>

        @can('users.create')
        <a href="{{ route('admin.users.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-medium transition-colors shadow-lg shadow-blue-500/30">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-900/50 text-slate-400 text-xs uppercase tracking-wider border-b border-white/5">
                        ...
                        <th class="px-6 py-4 font-medium">Foydalanuvchi</th>
                        <th class="px-6 py-4 font-medium">Email</th>
                        <th class="px-6 py-4 font-medium">Rol (Role)</th>
                    ...
                    <tr class="border-b border-white/5 hover:bg-slate-800/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center font-bold text-white uppercase">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-2">
                                ...
                                @empty
                                <span class="text-slate-500 italic text-xs">Rol biriktirilmagan</span>
                                @endforelse
                            </div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-3">
                                @can('users.edit')
                                <a href="{{ route('admin.users.edit', $user->id) }}" class="text-slate-400 hover:text-blue-400 transition-colors tooltip" title="Tahrirlash">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @can('users.delete')
                                @if(auth()->id() !== $user->id)
                                <button type="button" @click="deleteUrl = '{{ route('admin.users.destroy', $user->id) }}'; deleteModalOpen = true" class="text-slate-400 hover:text-rose-400 transition-colors tooltip" title="O'chirish">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
